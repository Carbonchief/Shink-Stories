...
- The required profile is `Schink Stories Luan iPhone Ad Hoc`, UUID `01b59745-5a1b-4380-9ab4-dc5a3a4b1381`. Its application identifier is `6DP8F4CY29.com.schink.stories.mobile` and it includes this iPhone's UDID. The matching signing identity is `Apple Distribution: SCHINK PTY. LTD. (6DP8F4CY29)` with codesign identity hash `3BA3C923A0ADEE31AA773C128A28C2ED62D0F7D5`.
- If the Ad Hoc profile is not installed locally, sign in to Apple Developer → Certificates, Identifiers & Profiles → Profiles with the Schink team account, download `Schink Stories Luan iPhone Ad Hoc`, and copy it to `~/Library/MobileDevice/Provisioning Profiles/01b59745-5a1b-4380-9ab4-dc5a3a4b1381.mobileprovision` immediately before the build. Never substitute a Personal Team, App Store provisioning profile, or another project profile.
- Clean stale MAUI intermediates before rebuilding when the packaged icon or other generated iOS assets do not match source: `dotnet clean Shink.Mobile/Shink.Mobile.csproj -f net10.0-ios -c Debug`.
- Build the current source for `net10.0-ios`, `Debug`, and `ios-arm64` with the Schink team, distribution identity, and profile explicitly supplied:
  - `dotnet build Shink.Mobile/Shink.Mobile.csproj -f net10.0-ios -c Debug -p:RuntimeIdentifier=ios-arm64 -p:ValidateXcodeVersion=false -p:CodesignKey='Apple Distribution: SCHINK PTY. LTD. (6DP8F4CY29)' -p:CodesignTeam=6DP8F4CY29 -p:CodesignProvision='Schink Stories Luan iPhone Ad Hoc'`
- Verify the build output reports `App Id: 6DP8F4CY29.com.schink.stories.mobile` and the `Schink Stories Luan iPhone Ad Hoc` profile. If the build fails only because of `com.apple.FinderInfo`, continue with the clean-copy/sign workflow below; do not treat that metadata failure as a source/build failure.
- macOS may reject the generated app bundle because of `com.apple.FinderInfo` metadata even after compiling it. In that case, make a fresh temporary copy with `ditto --norsrc --noextattr`, then re-sign the copy with the Schink distribution identity before installing. The re-signing entitlements must include the profile's `application-identifier`, team identifier, `get-task-allow=false`, and keychain-access groups, in addition to the app's Associated Domains and Sign in with Apple entitlements; otherwise iOS rejects the app.
  - `security cms -D -i ~/Library/MobileDevice/Provisioning\ Profiles/01b59745-5a1b-4380-9ab4-dc5a3a4b1381.mobileprovision -o /tmp/schink-adhoc-profile.plist`
  - `plutil -extract Entitlements xml1 -o /tmp/schink-adhoc.entitlements /tmp/schink-adhoc-profile.plist`
  - `ditto --norsrc --noextattr Shink.Mobile/bin/Debug/net10.0-ios/ios-arm64/Shink.Mobile.app /tmp/Shink.Mobile.clean.schink-adhoc.app`
  - `/usr/bin/codesign --force --sign 3BA3C923A0ADEE31AA773C128A28C2ED62D0F7D5 --entitlements /tmp/schink-adhoc.entitlements --timestamp=none /tmp/Shink.Mobile.clean.schink-adhoc.app`
  - `/usr/bin/codesign --verify --deep --strict /tmp/Shink.Mobile.clean.schink-adhoc.app`
