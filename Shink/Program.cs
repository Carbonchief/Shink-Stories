...
const string MobileGoogleCustomSchemeCallbackUrl = "schinkstories://auth/google";
    ...
    ILogger<Program> logger) =>
{
    ...
    if (string.Equals(recovery.SourceType, "store_order", StringComparison.OrdinalIgnoreCase))
    {
        ...
        var freshOrderItems = AddStoreDeliveryLineItem(order.Items.Select(item => new StoreOrderItemDraft(
            item.ProductSlug,
            item.ProductName,
            item.Quantity,
...
app.MapPost("/rekening/skuif-na-gratis", async (
    ISubscriptionLedgerService subscriptionLedgerService,
    HttpContext httpContext,
    ILogger<Program> logger) =>
{
    ...
    var signedInEmail = httpContext.User.FindFirst(ClaimTypes.Email)?.Value
                       ?? httpContext.User.Identity?.Name;
    var result = await subscriptionLedgerService.TransferPaidSubscriptionToGratisAsync(
        signedInEmail,
        httpContext.RequestAborted);
    if (!result.IsSuccess)
    {
        logger.LogWarning(
            "Paid-to-free transfer failed. email={Email} error={Error}",
            signedInEmail,
            result.ErrorMessage);
        return Results.Redirect(QueryHelpers.AddQueryString("/intekening-en-betaling", "rekening", "gratis-misluk"));
    }

    return Results.Redirect(QueryHelpers.AddQueryString("/luister", "rekening", "gratis"));
})
    ...
    ILogger<Program> logger) =>
{
    ...
    {
        logger.LogWarning(
            "Subscription repair could not resolve a fallback plan. email={Email} plan_slug={PlanSlug} error={Error}",
            signedInEmail,
    ...
    logger.LogWarning(
        "Subscription repair Paystack checkout fallback failed. email={Email} plan={PlanSlug} retry_error={RetryError} checkout_error={CheckoutError}",
        signedInEmail,
        plan.Slug,
        repairResult.ErrorMessage,
        checkoutResult.ErrorMessage);
    return Results.Redirect($"/betaal/{Uri.EscapeDataString(plan.Slug)}?provider=paystack&returnUrl={Uri.EscapeDataString("/luister")}");
})
.RequireRateLimiting("auth-submit")
.DisableAntiforgery();
...
app.MapPost("/api/paystack/webhook", HandlePaystackWebhookAsync)
.DisableAntiforgery();

app.MapPost("/wp-admin/admin-ajax.php", async (
    HttpContext httpContext,
    PaystackCheckoutService paystackCheckoutService,
    ISubscriptionLedgerService subscriptionLedgerService,
    IStoreOrderService storeOrderService,
    ...
    ILogger<Program> logger) =>
{
    ...
    if (!string.Equals(action, "pmpro_paystack_ipn", StringComparison.OrdinalIgnoreCase))
    {
        logger.LogWarning("Legacy WordPress admin-ajax webhook rejected: unsupported action={Action}.", action);
        return Results.NotFound();
    }

    logger.LogInformation("Legacy PMPro Paystack webhook endpoint received a request.");
    return await HandlePaystackWebhookAsync(
        httpContext,
        paystackCheckoutService,
        subscriptionLedgerService,
        storeOrderService,
...
app.MapGet("/api/mobile/auth/google/start", async (
    ...
    HttpContext httpContext) =>
{
    var useCustomSchemeCallback = IsMobileGoogleCustomSchemeCallback(callback);
    ...
    var protectedState = protector.Protect(JsonSerializer.Serialize(new GooglePkceStateCookiePayload(
        ...
        ReturnUrl: null,
        UseImplicitFlow: false,
        UseMobileCustomSchemeCallback: useCustomSchemeCallback)));
...
{
    GooglePkceStateCookiePayload? payload = null;
    var hasProtectedCookie = httpContext.Request.Cookies.TryGetValue(MobileGooglePkceCookieName, out var protectedCookie) &&
        !string.IsNullOrWhiteSpace(protectedCookie);
    ...
    var useCustomSchemeCallback = payload?.UseMobileCustomSchemeCallback == true;
    ClearMobileGooglePkceCookie(httpContext);

    if (!string.IsNullOrWhiteSpace(error))
    {
        return Results.Redirect(BuildMobileGoogleAuthCallbackUrl(token: null, errorMessage: error_description, useCustomSchemeCallback));
    }
    ...
    if (!exchangeResult.IsSuccess || string.IsNullOrWhiteSpace(exchangeResult.UserEmail))
    {
        return Results.Redirect(BuildMobileGoogleAuthCallbackUrl(token: null, errorMessage: exchangeResult.ErrorMessage, useCustomSchemeCallback));
    }
    ...
    return Results.Redirect(BuildMobileGoogleAuthCallbackUrl(mobileToken, errorMessage: null, useCustomSchemeCallback));
});

app.MapGet("/mobile-auth/google/callback", (HttpContext httpContext) =>
{
    return Results.Redirect(BuildMobileGoogleCustomSchemeCallbackUrl(httpContext.Request.QueryString));
});
    ...
    HttpContext httpContext) =>
{
    ...
    {
        return Results.Redirect(BuildPasswordRecoveryErrorRedirectPath("Die bevestiging-skakel word nie ondersteun nie. Probeer asseblief weer."));
    }

    if (string.IsNullOrWhiteSpace(token_hash))
    {
        return Results.Redirect(BuildPasswordRecoveryErrorRedirectPath("Die herstel-skakel is ongeldig of het verval. Vra asseblief 'n nuwe skakel aan."));
    }

    var exchangeResult = await supabaseAuthService.ExchangeRecoveryTokenHashAsync(
        token_hash,
        httpContext.RequestAborted);
    if (!exchangeResult.IsSuccess ||
        string.IsNullOrWhiteSpace(exchangeResult.AccessToken) ||
        string.IsNullOrWhiteSpace(exchangeResult.RefreshToken))
    {
        return Results.Redirect(BuildPasswordRecoveryErrorRedirectPath(
            exchangeResult.ErrorMessage ?? "Die herstel-skakel is ongeldig of het verval. Vra asseblief 'n nuwe skakel aan."));
    }

    var recoveryPath = ResolvePasswordRecoveryRedirectPath(next);
    return Results.Redirect(BuildPasswordRecoverySessionRedirectPath(
        recoveryPath,
    ...
    ISubscriptionLedgerService subscriptionLedgerService) =>
{
    ...
    return Results.Ok(new MobileHomeResponse(
        ...
        FreeStories: freeStoriesTask.Result
            .Select(story => BuildMobileStorySummary(httpContext, story, source: "gratis", isLocked: !CanAccessStory(story, access, "gratis"), isFavorite: false))
            .ToArray()));
}).DisableAntiforgery();
    ...
    bool compact = false) =>
{
    ...
    MobileStorySummaryResponse MapStory(StoryItem story) =>
        BuildMobileStorySummary(
            ...
            story,
            source: "luister",
            isLocked: !CanAccessStory(story, access, "luister"),
    ...
    IStoryTrackingService storyTrackingService) =>
{
    ...
    var signedInEmail = GetSignedInEmail(httpContext.User);
    var access = await ResolveMobileStoryAccessAsync(httpContext, subscriptionLedgerService);
    var isLocked = !CanAccessStory(story, access, normalizedSource);
    ...
    return Results.Ok(new MobileStoryDetailResponse(
        ...
        PreviousStory: previousStory is null ? null : BuildMobileStorySummary(httpContext, previousStory, normalizedSource, !CanAccessStory(previousStory, access, normalizedSource), favoriteSlugs.Contains(previousStory.Slug, StringComparer.OrdinalIgnoreCase)),
        NextStory: nextStory is null ? null : BuildMobileStorySummary(httpContext, nextStory, normalizedSource, !CanAccessStory(nextStory, access, normalizedSource), favoriteSlugs.Contains(nextStory.Slug, StringComparer.OrdinalIgnoreCase)),
    ...
    IStoryFavoriteService storyFavoriteService) =>
{
    ...
    var normalizedSource = string.Equals(request.Source, "gratis", StringComparison.OrdinalIgnoreCase)
        ? "gratis"
        : "luister";
    var storyPath = $"/luister/{Uri.EscapeDataString(normalizedStorySlug)}";
...
app.MapGet("/sitemap.xml", async (HttpContext httpContext, IStoryCatalogService storyCatalogService) =>
{
    ...
    paths.AddRange(freeStoriesTask.Result.Select(story => $"/luister/{Uri.EscapeDataString(story.Slug)}"));
...
{
    app.MapGet($"/{slug}", () => Results.Redirect($"/luister/{slug}", permanent: false));
}
...
if (args.Any(argument => string.Equals(argument, "--retry-overdue-paystack", StringComparison.OrdinalIgnoreCase)))
{
    using var scope = app.Services.CreateScope();
    ...
    app.Logger.LogInformation(
        ...
        result.FailedCount,
        result.SkippedRecentRetryCount,
    ...
    {
        app.Logger.LogInformation(
            "Paystack retry batch item. email={Email} subscription_id={SubscriptionId} charge_key={ChargeKey} status={AttemptStatus} reference={Reference} next_payment={NextPaymentDate} message={Message}",
            attempt.Email,
            attempt.SubscriptionId,
            attempt.ChargeKey,
            attempt.AttemptStatus,
            attempt.Reference,
            attempt.NextPaymentDateUtc,
            attempt.Message);
    }

    foreach (var error in result.Errors.Take(25))
    {
        app.Logger.LogWarning("Paystack retry batch issue: {Error}", error);
    }
...
static string ResolveStoryTrackingPath(string? storyPath, string storySlug, string source)
{
    ...
    {
        "gratis" => $"/luister/{Uri.EscapeDataString(storySlug)}",
    ...
    out string? errorMessage)
{
    ...
    var totalPriceZar = checkoutItems.Sum(item => item.LineTotalZar);
    ...
    bool deliveryFeeWaived = false)
{
    ...
    [
        .. items,
        new StoreOrderItemDraft(
            ProductSlug: StoreDeliveryProductSlug,
            ProductName: StoreDeliveryProductName,
            Quantity: 1,
            UnitPriceZar: StoreDeliveryFeeZar)
    ];
}
    ...
    ILogger<Program> logger)
{
    ...
    if (IsStorePaystackWebhookPayload(payload))
    {
        ...
        if (!storePersistResult.IsSuccess)
        {
            ...
            return Results.Problem(
                ...
                detail: storePersistResult.ErrorMessage,
                statusCode: StatusCodes.Status500InternalServerError);
        }
        else
        {
    ...
    if (!persistResult.IsSuccess)
    {
        ...
        return Results.Problem(
            title: "Paystack subscription webhook persistence failed",
            detail: persistResult.ErrorMessage,
            statusCode: StatusCodes.Status500InternalServerError);
    }
    else
    {
...
static bool IsMobileGoogleCustomSchemeCallback(string? callback)
{
    return string.Equals(callback, "custom-scheme", StringComparison.OrdinalIgnoreCase);
}

static string BuildMobileGoogleCustomSchemeCallbackUrl(QueryString queryString)
{
    return queryString.HasValue
        ? $"{MobileGoogleCustomSchemeCallbackUrl}{queryString.Value}"
        : MobileGoogleCustomSchemeCallbackUrl;
}

static string BuildMobileGoogleAuthCallbackUrl(string? token, string? errorMessage, bool useCustomSchemeCallback = false)
{
    var callbackUrl = useCustomSchemeCallback
        ? MobileGoogleCustomSchemeCallbackUrl
        : MobileGoogleCallbackUrl;

    if (!string.IsNullOrWhiteSpace(token))
    {
        return QueryHelpers.AddQueryString(callbackUrl, "token", token);
    }
    ...
    return QueryHelpers.AddQueryString(callbackUrl, "error", safeMessage);
}
...
static string BuildPasswordRecoveryErrorRedirectPath(string? message)
{
    var safeMessage = string.IsNullOrWhiteSpace(message)
        ...
        : message.Trim();
    var fragment = new QueryString()
        .Add("type", "recovery")
        .Add("error_description", safeMessage)
        .ToUriComponent()
        .TrimStart('?');

    return $"/herstel-wagwoord#{fragment}";
}

static string ResolvePasswordRecoveryRedirectPath(string? next)
{
    var safeNext = GetSafeAuthReturnUrl(next);
    return string.Equals(safeNext, "/account/update-password", StringComparison.OrdinalIgnoreCase)
        ? "/herstel-wagwoord"
        : safeNext is not null && safeNext.StartsWith("/herstel-wagwoord", StringComparison.OrdinalIgnoreCase)
            ? safeNext
            : "/herstel-wagwoord";
}

static string BuildPasswordRecoverySessionRedirectPath(
    string recoveryPath,
    string accessToken,
    string? refreshToken,
    string type)
{
    var fragment = new QueryString()
        ...
        .Add("refresh_token", refreshToken ?? string.Empty)
        .Add("type", type)
        .ToUriComponent()
        .TrimStart('?');

    return $"{recoveryPath}#{fragment}";
}
    ...
    ISubscriptionLedgerService subscriptionLedgerService)
{
    ...
    var paidTask = subscriptionLedgerService.HasActivePaidSubscriptionAsync(
        signedInEmail,
        httpContext.RequestAborted);
    var activeTierCodesTask = subscriptionLedgerService.GetActiveTierCodesAsync(
        signedInEmail,
        httpContext.RequestAborted);
    await Task.WhenAll(paidTask, activeTierCodesTask);

    return new MobileStoryAccess(
        IsAuthenticated: true,
        HasPaidSubscription: paidTask.Result,
        ActiveTierCodes: activeTierCodesTask.Result);
}

static bool CanAccessStory(StoryItem story, MobileStoryAccess access, string source) =>
    StoryAccessPolicy.ResolveRequirement(source, story) == StoryAccessRequirement.Free
        ? access.IsAuthenticated
        : StoryAccessPolicy.GetAllowedTierCodes(StoryAccessPolicy.ResolveRequirement(source, story))
            .Any(requiredTierCode => access.ActiveTierCodes?.Any(activeTierCode =>
                string.Equals(activeTierCode, requiredTierCode, StringComparison.OrdinalIgnoreCase)) == true);
...
static MobileStorySummaryResponse BuildMobileStorySummary(
    ...
    new(
        ...
        RequiresFullStoryAccess: StoryAccessPolicy.ResolveRequirement(source, story) == StoryAccessRequirement.AllStoriesOnly);
...
sealed record MobileStoryAccess(
    bool IsAuthenticated,
    bool HasPaidSubscription,
    IReadOnlyList<string>? ActiveTierCodes = null);
