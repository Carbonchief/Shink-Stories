...
daily_favorite_counts as (
    select
        ...
        count(*)::integer as total_favorites
    from favorite_events
    where favorited_at >= current_date - interval '13 days'
    group by 1
),
daily_activity as (
    select
...
top_stories as (
    select
        ...
        coalesce(story_listen_stats.total_listen_sessions, 0)::integer as total_listen_sessions,
        coalesce(story_listen_stats.total_listened_seconds, 0)::numeric as total_listened_seconds,
    ...
    where
        coalesce(story_view_stats.total_views, 0) > 0
        or coalesce(story_listen_stats.total_listen_sessions, 0) > 0
        or coalesce(story_favorite_stats.total_favorites, 0) > 0
    order by
        coalesce(story_listen_stats.total_listened_seconds, 0) desc,
        coalesce(story_view_stats.total_views, 0) desc,
        coalesce(story_favorite_stats.total_favorites, 0) desc,
...
top_characters as (
    select
        p.character_slug,
        coalesce(
            nullif(max(c.display_name), ''),
            initcap(replace(p.character_slug, '-', ' '))
        ) as display_name,
        count(*)::integer as total_audio_plays,
    ...
    left join public.story_characters as c
        on c.slug = p.character_slug
