...
const OORTJIES_CLICK_ENDPOINT = "/api/oortjies/click";
...
function forceLuisterPeekMascot(positionClass = "") {
    initializeOortjiesPeekMascot();
...
function trackOortjiesClick() {
    if (!(peekMascotElement instanceof HTMLElement)) {
        return;
    }

    const side = readCurrentPeekSide();
    ...
    if (typeof navigator.sendBeacon === "function") {
        ...
        if (navigator.sendBeacon(OORTJIES_CLICK_ENDPOINT, body)) {
            return;
        }
    }

    void fetch(OORTJIES_CLICK_ENDPOINT, {
        method: "POST",
        headers: {
        ...
        // Click analytics should never interrupt the peek-away animation.
    });
}

function readCurrentPeekSide() {
    if (!(peekMascotElement instanceof HTMLElement)) {
        return "";
    }

    if (peekMascotElement.classList.contains("peek-side-top")) {
        return "top";
    }

    if (peekMascotElement.classList.contains("peek-side-right")) {
        return "right";
    }

    if (peekMascotElement.classList.contains("peek-side-bottom")) {
        return "bottom";
    }

    if (peekMascotElement.classList.contains("peek-side-left")) {
        return "left";
    }
...
document.addEventListener("enhancedload", initializeOortjiesPeekMascot);
...
window.addEventListener("pageshow", initializeOortjiesPeekMascot);
window.addEventListener("popstate", scheduleHeaderInteractionsInitialization);
window.addEventListener("popstate", initializeOortjiesPeekMascot);
